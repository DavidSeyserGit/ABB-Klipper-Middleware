...
fn main() -> Result<(), Box<dyn std::error::Error>>{
    ...
    if let Some(extension) = path.extension(){
        if extension == "mod"{
            //we read a .mod file which is for ABB rapid code
            let contents = read_file(file_path)?;
            //add the socket to the string if it doesnt exist already
            let _ = fs::write(file_path, search_and_create_socket(contents));
            //we have to change every instance of CallExtruder to a socket  
        }
        else{
            todo!();
        }
